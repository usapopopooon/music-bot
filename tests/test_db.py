...
async def test_set_then_get_volume(database_url: str) -> None:
    db = await Database.connect(database_url, pool_size=2)
    try:
        await db.set_volume(guild_id=11111, bot_id=22222, volume=42)
        assert await db.get_volume(11111, 22222) == 42
        # Update on conflict
        await db.set_volume(guild_id=11111, bot_id=22222, volume=80)
        assert await db.get_volume(11111, 22222) == 80
    finally:
        await db.close()
...
async def test_volume_independent_per_bot(database_url: str) -> None:
    db = await Database.connect(database_url, pool_size=2)
    try:
        await db.set_volume(guild_id=11111, bot_id=1, volume=25)
        await db.set_volume(guild_id=11111, bot_id=2, volume=75)
        assert await db.get_volume(11111, 1) == 25
        assert await db.get_volume(11111, 2) == 75
    finally:
        await db.close()
...
async def test_volume_out_of_range_rejected(database_url: str) -> None:
    ...
    try:
        ...
        with pytest.raises(ValueError):
            await db.set_volume(guild_id=11111, bot_id=1, volume=101)
    finally:
        await db.close()
