...
Jockie Music を可能な限りコピーした Discord 音楽 Bot。
音量設定のみサーバーごとに永続化して変更可能とする（Jockie 無料版では音量変更は不可）。

---

## 1. ゴール

- **再現対象**: Jockie Music (公式 Discord 音楽 Bot) の挙動・コマンド体系・UX
- **差分（独自要素）**: ギルド単位の音量設定を PostgreSQL に保存し、`/volume` コマンドで自由に変更可能
...
- Jockie Music も内部的に Lavaplayer (Lavalink の音声エンジン) を使用していると公表されており、UX の同一性を担保するうえで一致が取りやすい。
...
- これは Jockie Music と同等の方式（Jockie も内部的に同じく YouTube 経由で再生）。
...
PostgreSQL に保存するのは「Bot 再起動後も保持したい設定値」のみ。キュー・現在再生位置などはメモリのみ（Jockie も再起動でキューは消える）。
...
| `volume` | `SMALLINT` | `NOT NULL DEFAULT 100`, `CHECK (volume BETWEEN 0 AND 200)` | 0–200 (%)。Lavalink へ渡す値そのもの |
...
- **読み取り**: ボットがボイスチャンネルに参加した直後に 1 回。`Player.volume` に反映。
...
- 行が無いときは「メモリ上は 100 として扱い、変更時に初めて INSERT」とする（書き込み回数を最小化）。
- 1 ギルドに複数ボットが招待されている場合、ボットごとに独立した音量を持つ（Jockie #1 と #2 で別音量）。
...
| 3 音量 | 🔉 −10 / 🎚️ 100% / 🔊 +10 | 中央のボタンはラベルが現在の音量を表示し、押下で「音量設定モーダル」を開く |
...
- 音量増減は ±10 単位、0–200 でクランプ。変更は **即 DB 保存**（§5.5）。
...
| 🎚️ Volume | `level` (必須, 0–200) | 整数 | 即時反映 + DB 保存（§5.5） |
...
- 範囲 `0`–`200`（無音 〜 倍率 2.0x）。Lavalink の volume フィルタは 0–1000 だが UX 上 200 を上限。
- 操作 UI:
  - 🔉 −10 / 🔊 +10（Control Panel 行 3）
  - 🎚️ <現在値>% 押下 → 数値モーダル
  - `/volume [level]`（slash フォールバック）
- **すべての音量変更で同一トランザクション内に DB 保存**（`INSERT ... ON CONFLICT DO UPDATE`）。
- 反映先: Lavalink Player の音量フィルタ。次回 `/play` 時はこの値で起動。

### 5.6 slash command フォールバック

UI から実行できる全操作を slash command でも提供する（モバイルの誤タップ対策・自動化用途・Jockie 互換）。
...
Jockie の `/filter` 相当。Phase 2 で Control Panel に「フィルタ」行を追加し、各フィルタを on/off するボタンを並べる予定。
...
- 再生終了 + キューが空 + ボイスに人間が 0 人になってから **5 分** で自動切断。
- 24/7 モード（Jockie の `/247`）は Phase 1 では非対応。
...
Jockie の `Jockie Music #1`〜`#4` 相当を、**1 つの Python プロセス内で複数の `discord.Client` を並走**させる方式で実現する。
...
- 既に Control Panel が表示されているチャンネルとは別チャンネルから `/play` した場合、振り分けで別 Client が選ばれる可能性がある（Jockie #1 と #2 が別チャンネルで同時稼働するイメージ）。
...
│       │   ├── voice.py      ← join / disconnect / 自動切断ハンドラ
...
3. もう一度 `Add Service` → 同じ GitHub Repo、サービス名 `lavalink`:
   ...
   - Settings → Deploy → Healthcheck Path **`/v4/info`**、Timeout 10s
...
| `lavalink` | **Healthcheck Path: `/v4/info`**、Timeout 10s。失敗 3 回でサービス再起動 |
...
- 同時再生数を抑える運用（DJ 1 人ルール、Player 自動切断短縮、`MAX_PLAYERS_PER_BOT` の絞り込み）が有効。
...
| `lavalink/railway.toml` | lavalink サービス | `lavalink/Dockerfile` | `/v4/info` |
...
`lavalink/railway.toml`:
```toml
[build]
builder = "dockerfile"
dockerfilePath = "lavalink/Dockerfile"
...
## 15. 仕様外（Jockie にあるが本 Bot では実装しないもの）

- prefix コマンド (`m!play` 等)
- DJ ロール / 投票スキップ
- プレミアム機能（`/savequeue` `/loadqueue` `/247` 等）
...
- Jockie Music コマンドリファレンス: https://jockiemusic.com (公式サイト)
